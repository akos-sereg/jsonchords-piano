...
export const PlayingContextProvider: FC = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (
      playingEpisode < data.episodes.length &&
      playingChord < data.episodes[playingEpisode].chords.length
    ) {
      return;
    }
  ...
  return <PlayingContext.Provider value={context}>{children}</PlayingContext.Provider>;
};
