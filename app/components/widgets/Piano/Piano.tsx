...
const Piano = ({ displayNotes }: PianoProps) => {
  ...
  const displayedNotes = displayNotes.map((note) => {
    ...
    if (fragments.length > 1 && fragments[1].includes('#')) {
      const offsetX =
        ratio * octaveImageOriginalWidth * (octavNumber - 1) +
        halfNoteOffsets[fragments[1].replace('#', '')] * ratio;
      return (
        <img src={pressed4} width={69 * ratio} style={{ position: 'absolute', left: offsetX }} />
      );
    } else {
      const offsetX =
        ratio * octaveImageOriginalWidth * (octavNumber - 1) +
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ width: 'fit-content', borderRight: 'solid 2px #000000' }}>
        {octaves.map((id, index) => (
          <img key={id} src={imgOctave} width={619 * ratio} />
        ))}
      </div>
      <div id="audio" />
    </div>
